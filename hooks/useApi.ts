    ...
    return {
        ...
        shopApi: {
            ...
            deleteMultiple: (ids: number[]) =>
                requestJson("DELETE", "/api/shop/bulk", ids),
        },
        productApi: {
            add: (shopId: number, data: any) =>
                requestJson("POST", `/api/shop/${shopId}/products`, data),
            getByShop: (shopId: number) =>
                requestJson("GET", `/api/shop/${shopId}/products`),
            update: (id: number, data: any) =>
                requestJson("PUT", `/api/shop/products/${id}`, data),
            delete: (id: number) => requestJson("DELETE", `/api/shop/products/${id}`),
            deleteMultiple: (ids: number[]) =>
                requestJson("DELETE", "/api/shop/products/bulk", ids),
        },
    };
}
