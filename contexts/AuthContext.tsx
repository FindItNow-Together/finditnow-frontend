...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const authRoutes = ["/login", "/signup", "/register"]; // Add your auth routes here
    if (isAuthenticated && authRoutes.includes(pathname)) {
      // Find the first route assigned to this role in your map
      const targetRoute = Object.entries(ROLE_ROUTE_MAP).find(([_, roles]) =>
        roles.includes(accessRole!)
      )?.[0];

      // Redirect to their dashboard (e.g., /admin, /shop, etc.) or home
